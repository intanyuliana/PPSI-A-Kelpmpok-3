...
<?php
    ...
    $sql2 = "SELECT jenis FROM jenis_industri where id_jenis = $getID";
    $res = $conn->query($sql2);
    $data2 = $res->fetch_assoc();

    $sql = "SELECT *FROM perusahaan a
            LEFT JOIN joblist b ON (a.id_perusahaan=b.id_perusahaan)
            LEFT JOIN bidang c ON (b.id_bidang=c.id_bidang)
            LEFT JOIN jenis_industri d ON (a.id_jenis=d.id_jenis)
            LEFT JOIN user e ON (a.id_user=e.id_user)
            LEFT JOIN kota f ON (f.id_kota=a.id_kota)
            WHERE a.id_jenis = $getID ";
      
    $res = $conn->query($sql);
    echo '<h3>'.$hasil.' Lowongan pekerjaan perusahaan : <span class="text-navy">"'.$data2['jenis'].'"</span></h3>';
